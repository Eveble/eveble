...
describe('SerializableError', () => {
  ...
  it(`extends ExtendableError`, () => {
    expect(SerializableError.prototype).toBeInstanceOf(ExtendableError);
  });

  it('implements Definable interface', () => {
    expect(derived(SerializableError.prototype, TypeTrait)).toBe(true);
    expect(instanceOf<types.Typed>(Serializable.prototype)).toBe(true);
  });

  it('implements Hookable interface', () => {
    expect(derived(SerializableError.prototype, HookableTrait)).toBe(true);
    expect(instanceOf<types.Hookable>(Serializable.prototype)).toBe(true);
  });

  it('implements Serializable interface', () => {
    expect(derived(SerializableError.prototype, SerializableTrait)).toBe(true);
    expect(instanceOf<types.Serializable>(Serializable.prototype)).toBe(true);
  });

  it('implements Ejsonable interface', () => {
    expect(derived(SerializableError.prototype, EjsonableTrait)).toBe(true);
  ...
  it('ensures that type is defined', () => {
    expect(isTyped(SerializableError.prototype)).toBe(true);
  });
  ...
  describe('property types', () => {
    it('returns property types as an object', () => {
      ...
      expect(MyNestedSerializableError.getPropTypes()).toEqual(propTypes);
      expect(error.getPropTypes()).toEqual(propTypes);
    });
  });
  ...
  describe('construction', () => {
    ...
    it('returns constructed parent class on valid properties', () => {
      ...
      expect(error).toBeInstanceOf(MyNestedSerializableError);
      expect(error).toBeInstanceOf(ExtendableError);
      expect(error.nested).toBeInstanceOf(MySerializable);
      expect(error.nested.foo).toBe(nested.foo);
    });
    ...
    it(`allows to set message as initializing class property`, () => {
      ...
      expect(new MyDefaultError().message).toBe(
    ...
    it('throws the prototype message by default', () => {
      ...
      }).toThrow(MySerializableError.prototype.message);
    });
  });
  ...
  describe('hooks', () => {
    it('supports onConstruction hook', () => {
      ...
      expect(instance.message).toBe('my-processed-error-message');
    });
  });
});
