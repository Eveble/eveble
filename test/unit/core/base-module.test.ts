...
describe('BaseModule', function() {
  ...
  describe('construction', () => {
    ...
    it('sets the constructed state on construction', () => {
      const module = new MyModule();
      expect(module.isInState(BaseModule.STATES.constructed)).to.be.true;
    });
  ...
  describe('initialization', () => {
    ...
    it('can be only initialized once', async () => {
      ...
      expect(module.isInState(BaseModule.STATES.initialized)).to.be.true;
  ...
  describe('starting', () => {
    ...
    it('sets the state to running', async () => {
      ...
      await module.initialize(app, injector);
      await module.start();
      expect(module.isInState(BaseModule.STATES.running)).to.be.true;
    ...
    it('allows to restart stopped module', async () => {
      ...
      expect(module.isInState(BaseModule.STATES.running)).to.be.true;
      await module.stop();
      expect(module.isInState(BaseModule.STATES.stopped)).to.be.true;
      await module.start();
      expect(module.isInState(BaseModule.STATES.running)).to.be.true;
    });
  ...
  describe('shutdown', () => {
    it('sets the state to shutdown after initialization of module', async () => {
      ...
      expect(module.isInState(BaseModule.STATES.shutdown)).to.be.true;
    });
  ...
  describe(`evaluation`, () => {
    describe(`environment`, () => {
      ...
      it(`returns true if environment is set to production`, () => {
        process.env.NODE_ENV = 'production';
        expect(BaseModule.prototype.isOnProduction()).to.be.true;
      });

      it(`returns false if environment is not set to production`, () => {
        expect(BaseModule.prototype.isOnProduction()).to.be.false;
      });
    });
  });
});
